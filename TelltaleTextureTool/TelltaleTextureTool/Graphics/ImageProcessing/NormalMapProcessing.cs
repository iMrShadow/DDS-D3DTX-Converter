...
namespace TelltaleTextureTool.ImageProcessing
{
    public static class NormalMapProcessing
    {
            ...
            string outputPath)
        {
            ...
            for (int x = 0; x < imgSharp.Width; x++) //make this cycle in a function which returns an image and then saves it ig
            {
                for (int y = 0; y < imgSharp.Height; y++)
                {
                    ...
                    imgSharp[x, y] = newFloatColor.GetColor();
                }
        ...
        /// <summary>
        /// Swizzles the mask of a normal map image and saves it to the disk.
        /// </summary>
        /// <param name="sourceFilePath"></param>
        /// <param name="outputPath"></param>
        public static void SwizzleMask(string sourceFilePath,
            string outputPath)
        {
            //open up the image
            using var image = Pfimage.FromFile(sourceFilePath);
