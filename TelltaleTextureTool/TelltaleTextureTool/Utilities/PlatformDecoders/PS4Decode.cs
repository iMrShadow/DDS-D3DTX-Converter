...
public static class PSTextureDecoder
{
    public static byte[] DecodePS4(byte[] pixelData, DXGIFormat format, int width, int height)
    {
        ...
        for (int i = 0; i < ((blockHeight + 7) / 8); i++)
        {
            for (int j = 0; j < ((blockWidth + 7) / 8); j++)
            {
                for (int k = 0; k < 64; k++)
                {
                    ...
                    if ((((j * 8) + num15) < blockWidth) && (((i * 8) + num14) < blockHeight))
                    {
                        int destinationIndex = count * (((((i * 8) + num14) * blockWidth) + (j * 8)) + num15);
